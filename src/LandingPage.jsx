...
const EN_STEPS = [
  ...
  { num: "05", desc: "Coco reviews your reading once payment is confirmed.", isBold: true },
  { num: "06", desc: "Your 1:1 — 60 / 90 mins reading session with Coco!" },
];

const Stars = () => {
  const positions = Array.from({ length: 80 }, (_, i) => ({
    x: ((i * 73.7) % 100), y: ((i * 43.3) % 100),
...
function AboutCoco({ isMobile, t }) {
  ...
  const pStyle = {
    ...
    color: "#a07840", lineHeight: 1.85,
    margin: "0 0 12px 0",
    textAlign: isMobile ? "center" : "left",
  };
  const closingStyle = { ...pStyle, color: "#c9a84c", fontStyle: "italic", margin: 0 };

  // Mobile: only one short sentence before Read more
  const para1Short = t
    ? "2020年是我人生的至暗时刻。我失去了事业，失去了爱情，疫情将我和最亲近的家人朋友远远隔绝。"
    : "2020 was the darkest time of my life. I lost my business, lost in love, and the pandemic locked me away from everyone I loved.";
        ...
        <div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 16 }}>
            <img
              ...
              alt="Coco Chen"
              style={{ width: 68, height: 68, borderRadius: "50%", border: "2px solid #c9a84c88", objectFit: "cover", flexShrink: 0 }}
            />
            <div>
              <div style={{ fontSize: 16, color: "#c9a84c", letterSpacing: 1, marginBottom: 6, fontFamily: "'Georgia', serif" }}>Coco Chen</div>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {["Tarot", "Birth Chart", "EN · 中文"].map(tag => (
                  <span key={tag} style={{ fontSize: 10, color: "#c9a84c88", letterSpacing: 1, borderBottom: "1px solid #c9a84c33", paddingBottom: 2 }}>{tag}</span>
                ))}
              </div>
            </div>
          </div>
          ...
          {/* Hook */}
          <p style={{ ...pStyle, color: "#c9a84c", fontWeight: "bold", fontStyle: "italic", fontSize: 15, marginBottom: 12 }}>{hook}</p>

          {/* Collapsed: one short para + Read more */}
          {!expanded ? (
            <>
              <p style={pStyle}>{para1Short}</p>
              <button onClick={() => setExpanded(true)} style={{
                ...
                {t ? t.landing.read_more : "Read more ↓"}
              </button>
            </>
          ) : (
            <>
              ...
              <button onClick={() => setExpanded(false)} style={{
                ...
                {t ? t.landing.show_less : "Show less ↑"}
              </button>
            </>
          )}
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "row", gap: 64, alignItems: "flex-start" }}>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8, flexShrink: 0 }}>
            <img
              src="https://res.cloudinary.com/da1asg0hq/image/upload/v1776685972/Gemini_Generated_Image_4cldof4cldof4cld_1_xjl6nl.png"
              alt="Coco Chen"
              style={{ width: 260, height: 260, borderRadius: "50%", border: "2px solid #c9a84c88", objectFit: "cover", display: "block" }}
            />
            <div style={{ textAlign: "center" }}>
              <div style={{ fontSize: 18, color: "#c9a84c", letterSpacing: 2, marginBottom: 10 }}>Coco Chen</div>
          ...
          <div style={{ flex: 1, maxWidth: 620 }}>
            <p style={{ ...pStyle, color: "#c9a84c", fontWeight: "bold", fontStyle: "italic", fontSize: 18, marginBottom: 16 }}>{hook}</p>
            <p style={pStyle}>{para1}</p>
            <p style={pStyle}>{para2}</p>
            <p style={pStyle}>{para3}</p>
            <p style={pStyle}>{para4}</p>
...
export default function LandingPage({ onBeginReading, onBeginChart, onBeginOracle, bgmOn, onToggleBgm, lang, onToggleLang }) {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setFadeIn(false);
      setTimeout(() => { setPhraseIdx(i => (i + 1) % 3); setFadeIn(true); }, 600);
    }, 2800);
  ...
  const STEPS = t
    ? t.landing.steps.map((desc, i) => ({ num: String(i+1).padStart(2,"0"), desc, isBold: i === 4 }))
    : EN_STEPS;
  ...
  return (
    <div style={{
      ...
      <style>{`
        ...
        .cta-btn { background:#c9a84c; border:none; border-radius:40px; color:#0d0221; font-family:'Georgia',serif; font-size:13px; font-weight:bold; letter-spacing:1px; padding:11px 22px; cursor:pointer; transition:all 0.3s ease; }
        .cta-btn:hover { background:linear-gradient(135deg,#c9a84c,#e8c96d,#c9a84c); background-size:200% auto; box-shadow:0 0 24px #c9a84c88,0 0 48px #c9a84c44; animation:shimmer 1.5s linear infinite; }
        .rose-btn { background:#c4a0a0 !important; color:#2a1212 !important; }
        .rose-btn:hover { background:linear-gradient(135deg,#c4a0a0,#ddbfbf,#c4a0a0) !important; background-size:200% auto !important; animation:shimmer 1.5s linear infinite !important; box-shadow:0 0 24px #c4a0a088,0 0 48px #c4a0a044 !important; }
        .lavender-btn { background:#b0a0c4 !important; color:#1a122a !important; }
        .lavender-btn:hover { background:linear-gradient(135deg,#b0a0c4,#cbbfdd,#b0a0c4) !important; background-size:200% auto !important; animation:shimmer 1.5s linear infinite !important; box-shadow:0 0 24px #b0a0c488,0 0 48px #b0a0c444 !important; }
        .step-card:hover { transform:translateY(-4px); }
        .step-card { transition:transform 0.3s ease; }
        .card-img { border-radius:10px; border:2px solid #c9a84c88; box-shadow:0 12px 40px #00000088; object-fit:cover; display:block; }
      `}</style>
      ...
      <section style={{
        ...
        {/* Card fan */}
        <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: isMobile ? 200 : 240, pointerEvents: "none", overflow: "hidden" }}>
          {(isMobile ? HERO_CARDS_MOBILE : HERO_CARDS_DESKTOP).map((card, i) => {
            const w = isMobile ? Math.round(card.w * 0.72) : card.w;
            const h = Math.round(w * 1.6);
            return (
              <img key={i} src={card.url} alt={`card ${i+1}`} className="card-img" style={{
                position: "absolute", left: `calc(50% + ${card.cx - w/2}px)`, top: card.y,
                width: w, height: h, zIndex: card.z,
          ...
          <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 160, background: "linear-gradient(to bottom, transparent, #0d0221)" }}/>
        </div>
        ...
        <div style={{ position: "relative", zIndex: 2, animation: "fadeUp 1s ease-out 0.4s both", width: "100%", maxWidth: isMobile ? 500 : 860, margin: "0 auto" }}>
          ...
          <div style={{ height: isMobile ? 80 : 100, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: isMobile ? 8 : 12 }}>
            <h1 style={{
              fontSize: isMobile ? "clamp(22px,6vw,28px)" : "clamp(28px,4vw,42px)",
              letterSpacing: isMobile ? 0.5 : 1, lineHeight: 1.3,
              fontFamily: "'IM Fell English', serif", color: "#e8d5b7",
              fontWeight: "normal", margin: 0,
              opacity: fadeIn ? 1 : 0, transition: "opacity 0.6s ease", textAlign: "center",
            }}>
              {PHRASES[phraseIdx]}
            </h1>
          </div>
          <p style={{ fontSize: isMobile ? "14px" : "17px", color: "#a07840", maxWidth: isMobile ? 500 : 800, margin: isMobile ? "0 auto 20px" : "0 auto 24px", lineHeight: 1.9, letterSpacing: 0.5, fontStyle: "italic" }}>
          ...
          <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
            <button className="cta-btn rose-btn" onClick={() => document.getElementById("about-coco").scrollIntoView({ behavior: "smooth" })}>
              {t ? t.landing.btn_about : "✨ About Coco"}
            </button>
            <button className="cta-btn lavender-btn" onClick={() => document.getElementById("how-it-works").scrollIntoView({ behavior: "smooth" })}>
      ...
      {/* ── ABOUT COCO ── */}
      <AboutCoco isMobile={isMobile} t={t} />

      {/* ── HOW IT WORKS ── */}
      <section id="how-it-works" style={{ padding: "40px 48px 80px", maxWidth: 720, margin: "0 auto", position: "relative", zIndex: 1 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", position: "relative" }}>
          {!isMobile && (
            <div style={{ position: "absolute", left: 39, top: 0, bottom: 0, width: 1, background: "linear-gradient(to bottom, transparent, #c9a84c44, transparent)" }}/>
          )}
          {STEPS.map((step, i) => (
            <div key={i} className="reveal step-card" style={{
              ...
              <div style={{ width: 96, flexShrink: 0, display: "flex", alignItems: "flex-start", justifyContent: "center", paddingTop: 4 }}>
                <div style={{ width: isMobile ? 40 : 48, height: isMobile ? 40 : 48, borderRadius: "50%", border: "1px solid #c9a84c", display: "flex", alignItems: "center", justifyContent: "center", background: "#0d0221", flexShrink: 0, position: "relative", zIndex: 1 }}>
      ...
      {/* ── FOOTER ── */}
      <div style={{ textAlign: "center", padding: "32px 24px", borderTop: "1px solid #7c5c2e22", position: "relative", zIndex: 1 }}>
      ...
      {/* Lang toggle */}
      {onToggleLang && (
        <button onClick={onToggleLang} style={{ position: "fixed", top: 20, right: 72, height: 44, borderRadius: 22, padding: "0 16px", background: "#1a0545", border: "1px solid #c9a84c", color: "#c9a84c", fontSize: 13, cursor: "pointer", zIndex: 999, fontFamily: "'Georgia', serif", letterSpacing: 1, transition: "all 0.3s ease" }}>
          {lang === "en" ? "中文" : "EN"}
        </button>
      )}
      ...
      {/* Music toggle */}
      {onToggleBgm && (
        <button onClick={onToggleBgm} style={{ position: "fixed", top: 20, right: 20, width: 44, height: 44, borderRadius: "50%", background: bgmOn ? "#1f3a1f" : "#1a0545", border: "1px solid #c9a84c", color: "#c9a84c", fontSize: 18, cursor: "pointer", zIndex: 999, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: bgmOn ? "0 0 12px #c9a84c44" : "none", transition: "all 0.3s ease" }} title={bgmOn ? "Music ON" : "Music OFF"}>
          {bgmOn ? "🔊" : "🔇"}
        </button>
      )}
    </div>
