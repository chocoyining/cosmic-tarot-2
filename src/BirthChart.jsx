...
const LOCATION_DB = {
  ...
  "Singapore": { dst: false, cities: [{ label: "Singapore", lat: 1.3521, lon: 103.8198, tz: 8 }]},
  "Indonesia": { dst: false, cities: [
  ...
  "Hong Kong": { dst: false, cities: [{ label: "Hong Kong", lat: 22.3193, lon: 114.1694, tz: 8 }]},
  "Macau":     { dst: false, cities: [{ label: "Macau",     lat: 22.1987, lon: 113.5439, tz: 8 }]},
  "China": { dst: false, cities: [
    ...
    { label: "Xian",      lat: 34.3416, lon: 108.9398, tz: 8 },
  ...
  "United Kingdom": { dst: true, cities: [
  ...
  "New Zealand":  { dst: true,  cities: [{ label: "Auckland",  lat: -36.8509, lon: 174.7645, tz: 12 },{ label: "Wellington", lat: -41.2865, lon: 174.7762, tz: 12 }]},
  "Germany":      { dst: true,  cities: [{ label: "Berlin",    lat: 52.5200,  lon:  13.4050, tz:  1 },{ label: "Munich",     lat: 48.1351,  lon:  11.5820, tz:  1 },{ label: "Frankfurt",  lat: 50.1109,  lon:   8.6821, tz:  1 }]},
  "France":       { dst: true,  cities: [{ label: "Paris",     lat: 48.8566,  lon:   2.3522, tz:  1 },{ label: "Lyon",       lat: 45.7640,  lon:   4.8357, tz:  1 }]},
  "UAE":          { dst: false, cities: [{ label: "Dubai",     lat: 25.2048,  lon:  55.2708, tz:  4 },{ label: "Abu Dhabi",  lat: 24.4539,  lon:  54.3773, tz:  4 }]},
  "Saudi Arabia": { dst: false, cities: [{ label: "Riyadh",    lat: 24.7136,  lon:  46.6753, tz:  3 },{ label: "Jeddah",     lat: 21.3891,  lon:  39.8579, tz:  3 }]},
  "South Africa": { dst: false, cities: [{ label: "Johannesburg", lat: -26.2041, lon: 28.0473, tz: 2 },{ label: "Cape Town", lat: -33.9249, lon: 18.4241, tz: 2 }]},
};
...
function calcChart(year,month,day,hour,minute,tz,lat,lon) {
  const origin=new Origin({year,month:month-1,date:day,hour,minute,latitude:lat,longitude:lon});
  const horoscope=new Horoscope({origin,houseSystem:"placidus",zodiac:"tropical",aspectPoints:[],aspectWithPoints:[],aspectTypes:[]});
  const KEYS=[{key:"sun",name:"Sun"},{key:"moon",name:"Moon"},{key:"mercury",name:"Mercury"},{key:"venus",name:"Venus"},{key:"mars",name:"Mars"},{key:"jupiter",name:"Jupiter"},{key:"saturn",name:"Saturn"},{key:"uranus",name:"Uranus"},{key:"neptune",name:"Neptune"},{key:"pluto",name:"Pluto"}];
  const asc=horoscope.Ascendant;
  const aD=asc.ChartPosition.Ecliptic.DecimalDegrees;
  const results=[{planet:"Rising",sign:asc.Sign.label,signGlyph:getSignGlyph(asc.Sign.label),degrees:Math.floor(aD%30),minutes:Math.floor(((aD%30)-Math.floor(aD%30))*60),house:1}];
  for(const{key,name}of KEYS){
    try{
      const b=horoscope.CelestialBodies[key];
      const d=b.ChartPosition.Ecliptic.DecimalDegrees;
      const deg=Math.floor(d%30);
      const min=Math.floor(((d%30)-deg)*60);
      results.push({planet:name,sign:b.Sign.label,signGlyph:getSignGlyph(b.Sign.label),degrees:deg,minutes:min,house:b.House?b.House.id:1});
    }catch(e){results.push({planet:name,sign:"?",signGlyph:"?",degrees:0,minutes:0,house:0});}
  }
  return results;
}
...
const bgStyle={minHeight:"100vh",background:"linear-gradient(160deg,#0d0221 0%,#1a0545 40%,#2d0b6b 70%,#0d0221 100%)",fontFamily:"'Georgia', serif",color:"#e8d5b7",padding:"32px 16px",position:"relative",overflow:"hidden"};
const inputStyle={background:"#ffffff0d",border:"1px solid #7c5c2e",borderRadius:8,color:"#e8d5b7",padding:"10px 14px",fontSize:14,width:"100%",fontFamily:"'Georgia', serif",outline:"none",minHeight:48};
const selectStyle={...inputStyle,cursor:"pointer"};
function btn(bg,extra={}){return{background:bg,border:"1px solid #7c5c2e",borderRadius:8,color:"#e8d5b7",padding:"13px 0",fontSize:15,cursor:"pointer",letterSpacing:1,fontFamily:"'Georgia', serif",width:"100%",...extra};}

const Stars=()=>(
  <svg style={{position:"fixed",top:0,left:0,width:"100%",height:"100%",pointerEvents:"none",zIndex:0}} xmlns="http://www.w3.org/2000/svg">
    {Array.from({length:60},(_,i)=>(<circle key={i} cx={`${(i*37.7)%100}%`} cy={`${(i*23.3)%100}%`} r={i%5===0?1.5:1} fill="#fff" opacity={0.3+(i%5)*0.1}/>))}
  </svg>
);
const Label=({children})=>(<div style={{fontSize:11,color:"#a07840",letterSpacing:2,marginBottom:6}}>{children}</div>);
...
export default function BirthChart({onHome,lang,onToggleLang}){
  ...
  function handleCountryChange(e){setCountry(e.target.value);setCityIdx(0);setDst(false);setChart(null);}

  async function geocodeCity(){
    if(!customCity.trim())return;
    setCustomLoading(true);setCustomError("");
    try{
      const res=await fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(customCity)}&format=json&limit=1`,{headers:{"Accept-Language":"en"}});
      const data=await res.json();
      if(!data||data.length===0){setCustomError("City not found. Please try a different spelling.");setCustomLoading(false);return;}
      const lat=parseFloat(data[0].lat),lon=parseFloat(data[0].lon);
      const tz=Math.max(-12,Math.min(14,Math.round(lon/15)));
      setCountry("Custom");setCityIdx(0);
      LOCATION_DB["Custom"]={dst:false,cities:[{label:customCity,lat,lon,tz}]};
      setCustomError("");setCustomLoading(false);
    }catch(e){setCustomError("Could not look up city. Please check your connection.");setCustomLoading(false);}
  }

  async function trySaveOrShare(jpgData,filename,name){
    const isMobile=navigator.maxTouchPoints>0&&/Mobi|Android|iPhone|iPad/i.test(navigator.userAgent);
    if(isMobile&&navigator.share&&navigator.canShare){
      try{const blob=await(await fetch(jpgData)).blob();const file=new File([blob],filename,{type:"image/jpeg"});if(navigator.canShare({files:[file]})){await navigator.share({files:[file],title:`${name||"My"} Cosmic Blueprint`});return;}}catch(e){}
    }
    const link=document.createElement("a");link.download=filename;link.href=jpgData;link.click();
  }

  async function captureAndUpload(){
    const canvas=await window.html2canvas(captureRef.current,{useCORS:true,allowTaint:true,backgroundColor:"#0d0221",scale:2,width:480,windowWidth:480});
    ...
    try{
      const blob=await(await fetch(jpgData)).blob();const fd=new FormData();fd.append("file",blob,filename);fd.append("upload_preset",CLOUDINARY_PRESET);
      const r=await fetch(`https://api.cloudinary.com/v1_1/${CLOUDINARY_CLOUD}/image/upload`,{method:"POST",body:fd});
      const d=await r.json();imageUrl=d.secure_url||`Upload error: ${JSON.stringify(d.error||d)}`;
    }catch(e){imageUrl=`Upload exception: ${e.message}`;}
    return{filename,jpgData,imageUrl};
  }

  async function sendAndSaveChart(){
    if(!captureRef.current)return;
    try{
      const{filename,jpgData,imageUrl}=await captureAndUpload();
      if(window.emailjs){
        await window.emailjs.send(EMAILJS_SERVICE_ID,EMAILJS_TEMPLATE_ID,{
          client_name:name||"Unknown",client_dob:day&&month&&year?`${day}/${month}/${year}`:"Not provided",
          client_contact:clientContact||"Not provided",spread_name:"Cosmic Blueprint (Birth Chart)",
          client_question:`${city.label}, ${country}`,sent_at:new Date().toLocaleString("en-GB"),spread_image:imageUrl,
        });
      }
      await trySaveOrShare(jpgData,filename,name);showToast();
    }catch(e){alert("Something went wrong. Please try again.");}
  }

  async function saveChart(){
    if(!captureRef.current)return;
    try{const{filename,jpgData}=await captureAndUpload();await trySaveOrShare(jpgData,filename,name);showToast();}
  ...
  function calculate(){
    ...
    setError("");
    try{
      const result=calcChart(parseInt(year),parseInt(month),parseInt(day),parseInt(hour),parseInt(minute),effectiveTz,city.lat,city.lon);
      setChart(result);setScreen("result");window.scrollTo(0,0);
    }catch(e){setError("Calculation error: "+e.message);}
  }

  // ── RESULT ──────────────────────────────────────────────────────────────────
  if(screen==="result"&&chart){
    const colsD="1.4fr 1.8fr 0.8fr";
    const colsM="1fr 1fr 38px";
    return(
      <div style={bgStyle}>
        <Stars/>
        ...
        {/* Hidden capture */}
        <div ref={captureRef} style={{position:"fixed",left:-9999,top:0,background:"linear-gradient(160deg,#0d0221 0%,#1a0545 40%,#2d0b6b 70%,#0d0221 100%)",padding:"28px 20px 32px",width:480,display:"flex",flexDirection:"column",alignItems:"center"}}>
        ...
        <div style={{maxWidth:560,margin:"0 auto",position:"relative",zIndex:1}}>
          <div style={{textAlign:"center",marginBottom:desktop?24:14,animation:"fadeInUp 0.5s ease-out"}}>
            <div style={{fontSize:11,color:"#7a5a3a",letterSpacing:2,marginBottom:4}}>{t?t.chart.result_for:"COSMIC BLUEPRINT FOR"}</div>
            <div style={{fontSize:desktop?26:20,color:"#c9a84c",letterSpacing:2,marginBottom:2}}>{name||"My Chart"}</div>
            <div style={{fontSize:desktop?13:11,color:"#7a5a3a"}}>{day}/{month}/{year} · {String(hour).padStart(2,"0")}:{String(minute).padStart(2,"0")} · {city.label}, {country}</div>
          </div>

          {/* Table */}
          <div style={{background:"#ffffff08",border:"1px solid #7c5c2e44",borderRadius:16,overflow:"hidden",marginBottom:desktop?20:14,animation:"fadeInUp 0.5s ease-out"}}>
            {/* Headers */}
            <div style={{display:"grid",gridTemplateColumns:desktop?colsD:colsM,background:"#ffffff0a",padding:desktop?"12px 20px":"8px 10px",borderBottom:"1px solid #7c5c2e44"}}>
              {[t?t.chart.planet_header:"PLANET",t?t.chart.sign_header:"SIGN",desktop?(t?t.chart.house_header:"HOUSE"):"HSE"].map((h,i)=>(
                <div key={h} style={{fontSize:desktop?11:9,color:"#a07840",letterSpacing:desktop?2:1,textAlign:i===2?"right":i===1?"center":"left"}}>{h}</div>
              ))}
            </div>
            {/* Rows */}
            {chart.map(({planet,sign,signGlyph,degrees,minutes,house},idx)=>(
              <div key={planet} style={{display:"grid",gridTemplateColumns:desktop?colsD:colsM,padding:desktop?"14px 20px":"7px 10px",borderBottom:idx<chart.length-1?"1px solid #7c5c2e22":"none",background:!desktop&&planet==="Rising"?"#c9a84c0d":"transparent",alignItems:"center",animation:`fadeInUp 0.4s ease-out ${idx*0.05}s both`}}>
                <div style={{display:"flex",alignItems:"center",gap:desktop?10:5}}>
                  <span style={{fontSize:desktop?20:13,lineHeight:1}}>{PLANET_GLYPHS[planet]}</span>
                  <span style={{fontSize:desktop?14:11,color:PLANET_COLORS[planet],fontWeight:!desktop&&planet==="Rising"?"bold":"normal"}}>
                    {t&&t.chart.planets[planet]?t.chart.planets[planet]:planet}
                  </span>
                </div>
          ...
          {desktop?(
            <div style={{display:"flex",gap:12,flexWrap:"wrap",justifyContent:"center",marginBottom:16}}>
              <button onClick={sendAndSaveChart} style={{...btn("#5a0e3a"),width:"auto",padding:"11px 28px",fontSize:13}}>{t?t.chart.btn_send:"✦ Send My Chart to Coco"}</button>
              <button onClick={saveChart}        style={{...btn("#5a2e0e"),width:"auto",padding:"11px 28px",fontSize:13}}>{t?t.chart.btn_save:"📸 Save My Chart"}</button>
              <button onClick={()=>{setScreen("form");setChart(null);window.scrollTo(0,0);}} style={{...btn("#2a1a2a"),width:"auto",padding:"11px 28px",fontSize:13}}>{t?t.chart.btn_recalculate:"← Recalculate"}</button>
              <button onClick={onHome} style={{...btn("#2a1a1a"),width:"auto",padding:"11px 28px",fontSize:13}}>{t?t.chart.btn_home:"⌂ Home"}</button>
              {onToggleLang&&<button onClick={onToggleLang} style={{...btn("#2a1a40"),width:"auto",padding:"11px 20px",fontSize:12}}>{lang==="en"?"中文":"EN"}</button>}
            </div>
          ):(
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              <button onClick={sendAndSaveChart} style={{background:"#5a0e3a",border:"1px solid #7c5c2e",borderRadius:10,color:"#e8d5b7",padding:"15px",fontSize:13,cursor:"pointer",letterSpacing:1,fontFamily:"'Georgia',serif",width:"100%"}}>
                {t?t.chart.btn_send:"✦ Send My Chart to Coco"}
              </button>
              <div style={{display:"flex",gap:8}}>
                <button onClick={saveChart} style={{flex:1,background:"#5a2e0e",border:"1px solid #7c5c2e",borderRadius:8,color:"#e8d5b7",padding:"12px 8px",fontSize:12,cursor:"pointer",fontFamily:"'Georgia',serif"}}>📸 Save</button>
                <button onClick={()=>{setScreen("form");setChart(null);window.scrollTo(0,0);}} style={{flex:1,background:"#2a1a2a",border:"1px solid #7c5c2e",borderRadius:8,color:"#a07840",padding:"12px 8px",fontSize:12,cursor:"pointer",fontFamily:"'Georgia',serif"}}>← Redo</button>
                <button onClick={onHome} style={{flex:1,background:"#2a1a1a",border:"1px solid #7c5c2e",borderRadius:8,color:"#a07840",padding:"12px 8px",fontSize:12,cursor:"pointer",fontFamily:"'Georgia',serif"}}>⌂ Home</button>
              </div>
              {onToggleLang&&<button onClick={onToggleLang} style={{background:"#2a1a40",border:"1px solid #7c5c2e",borderRadius:8,color:"#c9a84c",padding:"11px",fontSize:12,cursor:"pointer",fontFamily:"'Georgia',serif",width:"100%"}}>{lang==="en"?"中文 · Switch to Chinese":"EN · Switch to English"}</button>}
            </div>
          )}
        </div>

        <style>{`@keyframes toastIn{from{opacity:0;transform:translateX(-50%) translateY(20px)}to{opacity:1;transform:translateX(-50%) translateY(0)}} @keyframes toastOut{from{opacity:1}to{opacity:0}}`}</style>
        {toast&&(<div style={{position:"fixed",bottom:40,left:"50%",transform:"translateX(-50%)",background:"linear-gradient(135deg,#2d0b6b,#1a0545)",border:"1px solid #c9a84c88",borderRadius:32,padding:"12px 24px",fontSize:13,color:"#c9a84c",letterSpacing:0.5,textAlign:"center",zIndex:999,boxShadow:"0 4px 24px #0009",animation:"toastIn 0.4s ease-out, toastOut 0.6s ease-in 4.3s forwards",whiteSpace:"nowrap"}}>✦ Successfully sent to Coco! You will be contacted soon for an exquisite reading ✦</div>)}
      </div>
    );
  }
    ...
    <div style={bgStyle}>
      ...
      <div style={{maxWidth:640,margin:"0 auto",position:"relative",zIndex:1}}>
        <div style={{textAlign:"center",marginBottom:32}}>
          <div style={{fontSize:desktop?28:22,color:"#c9a84c",letterSpacing:3,marginBottom:14}}>{t?"✦ 你的星座命盘 ✦":"✦ Your Cosmic Blueprint ✦"}</div>
          <div style={{fontSize:13,color:"#a07840",lineHeight:1.9,maxWidth:500,margin:"0 auto"}}>
            {t?(<>每个人都诞生于一个独特的时刻。那一刻，行星以特定的方式排列于天空之中。<br/><br/><em style={{color:"#c9a84c88"}}>把它想象成一面镜子，它将映照出最真实的你。</em></>):(<>Every person is born at a unique moment in time, when the planets were arranged in a specific pattern across the sky. This arrangement — your natal chart — is like a cosmic fingerprint.<br/><br/><em style={{color:"#c9a84c88"}}>Think of it as a mirror, one that reflects your truest self.</em></>)}
          </div>
        </div>

        <div style={{background:"#ffffff08",border:"1px solid #7c5c2e44",borderRadius:16,padding:desktop?"32px 36px":"20px 16px",marginBottom:24}}>
          <div style={{display:"flex",flexDirection:"column",gap:desktop?20:16}}>
            ...
            <div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr",gap:8}}>
                <input style={inputStyle} type="number" placeholder={desktop?"Year (e.g. 1993)":"Year"} value={year}  onChange={e=>setYear(e.target.value)}  min="1900" max="2099"/>
                <input style={inputStyle} type="number" placeholder="MM" value={month} onChange={e=>setMonth(e.target.value)} min="1" max="12"/>
                <input style={inputStyle} type="number" placeholder="DD" value={day}   onChange={e=>setDay(e.target.value)}   min="1" max="31"/>
              </div>
            </div>
            ...
            <div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                <select style={selectStyle} value={hour}   onChange={e=>setHour(e.target.value)}>
                  {Array.from({length:24},(_,i)=><option key={i} value={i} style={{background:"#1a0545"}}>{String(i).padStart(2,"0")}:00</option>)}
                </select>
                <select style={selectStyle} value={minute} onChange={e=>setMinute(e.target.value)}>
                  {Array.from({length:60},(_,i)=><option key={i} value={i} style={{background:"#1a0545"}}>: {String(i).padStart(2,"0")}</option>)}
                </select>
              </div>
            ...
            <div>
              ...
              <div style={{fontSize:11,color:"#7a5a3a",marginTop:6}}>{t?"时区：":"Timezone: "}UTC{effectiveTz>=0?"+":""}{effectiveTz} · {t?"自动套用":"auto-applied"}</div>
            </div>
            ...
            <div style={{background:"#ffffff08",border:"1px solid #7c5c2e44",borderRadius:10,padding:"14px 16px"}}>
              ...
              <div style={{display:"flex",gap:8}}>
                <input style={{...inputStyle,flex:1}} type="text" placeholder="e.g. Brentwood, CA, USA" value={customCity} onChange={e=>setCustomCity(e.target.value)} onKeyDown={e=>e.key==="Enter"&&geocodeCity()}/>
                <button onClick={geocodeCity} disabled={customLoading} style={{background:"#3b1f6e",border:"1px solid #7c5c2e",borderRadius:8,color:"#e8d5b7",padding:"10px 14px",fontSize:13,cursor:"pointer",fontFamily:"'Georgia',serif",whiteSpace:"nowrap",flexShrink:0,minHeight:48}}>
                  {customLoading?"...":(t?t.chart.custom_search:"Search")}
                </button>
              </div>
              {customError&&<div style={{color:"#c94c4c",fontSize:11,marginTop:6}}>{customError}</div>}
              {country==="Custom"&&!customError&&customCity&&<div style={{color:"#a8d8a8",fontSize:11,marginTop:6}}>✓ Found: {city.label} (UTC{city.tz>=0?"+":""}{city.tz})</div>}
            </div>
            ...
            {showDst&&(
              <div style={{background:"#ffffff08",border:"1px solid #7c5c2e44",borderRadius:10,padding:"14px 16px"}}>
                ...
                <div style={{fontSize:12,color:"#7a5a3a",marginBottom:12,lineHeight:1.6}}>
                  {(country==="United States"||country==="Canada")?(t?t.chart.dst_us:"US/Canada: DST runs March to November."):(country==="Australia"||country==="New Zealand")?(t?t.chart.dst_au:"AU/NZ: DST runs October to April."):(t?t.chart.dst_other:"This country observes DST in summer months.")}{" "}{t?t.chart.dst_question:"Were you born during this period?"}
                </div>
                <div style={{display:"flex",gap:8}}>
                  {[t?t.chart.dst_no:"No (Standard Time)",t?t.chart.dst_yes:"Yes (Daylight Saving)"].map((label,i)=>(
                    <button key={i} onClick={()=>setDst(i===1)} style={{flex:1,padding:"10px 0",borderRadius:8,fontSize:12,cursor:"pointer",fontFamily:"'Georgia',serif",minHeight:44,background:dst===(i===1)?"#3b1f6e":"#ffffff0d",border:`1px solid ${dst===(i===1)?"#c9a84c":"#7c5c2e"}`,color:dst===(i===1)?"#c9a84c":"#a07840"}}>{label}</button>
                  ))}
                </div>
              </div>
            )}
            ...
            <div>
              ...
              <input style={{...inputStyle,borderColor:contactError?"#c94c4c":"#7c5c2e"}} type="text" placeholder={t?t.chart.contact_placeholder:"Fill in to receive your reading from Coco"} value={clientContact} onChange={e=>{setClientContact(e.target.value);setContactError(false);}}/>
              {contactError&&<div style={{color:"#c94c4c",fontSize:11,marginTop:4}}>Please enter your email or WhatsApp to continue.</div>}
            </div>

            <button onClick={calculate} style={{...btn("#3b1f6e"),minHeight:52,fontSize:desktop?15:14}}>
              {t?t.chart.btn_calculate:"✦ Calculate My Chart"}
            </button>
        ...
        <div style={{textAlign:"center",marginBottom:16}}>
          <button onClick={onHome} style={{...btn("#2a1a1a"),width:"auto",padding:"10px 32px",fontSize:13}}>⌂ Home</button>
        </div>
      </div>
    </div>
  );
